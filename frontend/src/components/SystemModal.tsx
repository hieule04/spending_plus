        ...
        <div className="p-6 space-y-6">
          <div className={`p-5 ${sectionClass}`}>
            <h3 className={`text-lg font-semibold mb-3 flex items-center gap-2 ${headingClass}`}>
              <span className="w-2.5 h-2.5 rounded-full bg-blue-500 animate-pulse"></span>{t('system.backend')}
            </h3>
            {loading ? (
          ...
          <div className={`p-5 ${sectionClass}`}>
            <h3 className={`text-lg font-semibold mb-3 ${headingClass}`}>{t('system.db_check')}</h3>
            <button onClick={testDatabase} className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-all shadow-lg active:scale-95 shadow-blue-500/20">{t('system.db_test_btn')}</button>
            {dbStatus && (
